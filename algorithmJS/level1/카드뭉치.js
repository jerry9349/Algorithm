...
function solution(cards1, cards2, goal) {
  if (cards1.length + cards2.length < goal) return "No";
  let arr = [];
  ...
  return arr.join('').indexOf(goal.join('')) > -1 ? "Yes" : "No";
}
...
function solution(cards1, cards2, goal) {
  while (goal.length !== 0) {
    let target = goal[0];
    if (cards1.length >= 1 && cards1[0] === target) { cards1.shift(); goal.shift() }
    else if (cards2.length >= 1 && cards2[0] === target) { cards2.shift(); goal.shift() }
    else break;
  }
