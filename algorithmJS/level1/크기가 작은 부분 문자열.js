...
function solution(t, p) {
  ...
  for (let i = 0; i < t.length; i++) {
    let tNumber = arr.filter((_, idx) => idx > i - 1 && idx < i + p.length).join('');
    if (tNumber.length !== p.length) break;
    if (parseInt(tNumber) <= parseInt(p)) count++;
  }
  return count;
}
